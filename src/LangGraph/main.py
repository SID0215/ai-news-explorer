...
def load_app():
    ...
    st.session_state["NEWS_TYPE"] = user_input.get("NEWS_TYPE", "news")


    if not user_input:
        st.error("Error: Failed to load user input from ui")
        return
    if st.session_state.IsFetchButtonClicked or st.session_state.IsFetchAIButtonClicked:
        user_message=st.session_state.timeframe
    else:
        user_message = st.chat_input("Enter the messsage:")

    print(user_input['NEWS_TYPE'])
    ...
    if user_message:
        try:
            ...
            graph_builder=GraphBuilder(model,user_input['NEWS_TYPE'])
            try:
